...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        ...
        "create context anchors at user/tool message boundaries (default: disabled)",
        [](common_params & params) {
            params.semantic_checkpoints = true;
        }
    ).set_env("LLAMA_ARG_SEMANTIC_CHECKPOINTS").set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--semantic-checkpoints-on-device"},
        "keep the newest semantic context anchor in a device-resident state buffer (default: disabled)",
        [](common_params & params) {
            params.semantic_checkpoints = true;
            params.semantic_checkpoints_on_device = true;
        }
    ).set_env("LLAMA_ARG_SEMANTIC_CHECKPOINTS_ON_DEVICE").set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"-cram", "--cache-ram"}, "N",
